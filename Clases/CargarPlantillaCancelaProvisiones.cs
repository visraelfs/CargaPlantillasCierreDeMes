...
namespace CargaPlantillasCierreDeMes.Clases
{
    ...
    internal class CargarPlantillaCancelaProvisiones
    {
        public string _rutaArchivo { get; set; }
        private Action<int> _reportProgress;

        public CargarPlantillaCancelaProvisiones(string rutaArchivo, Action<int> reportProgress)
        {
        ...
        public bool CargarPlantilla()
        {
            ...
            {
                _reportProgress(1);
                List<ExcelCancelaProvisiones> lregistrosExcel = ObtenerInformacionArchivo();

                var encontrarDuplicados = lregistrosExcel.GroupBy(x => new { x.LineItemId, x.Mes, x.Anio, x.NoFacSae }).Where(g => g.Count() > 1).ToList();
                ...
                if (encontrarDuplicados.Any())
                {
                    ...
                    foreach (var grupo in encontrarDuplicados)
                    {
                        ...
                        registrosDuplicados.AppendLine($"Producto: {grupo.Key.LineItemId}, mes: {grupo.Key.Mes}, año: {grupo.Key.Anio}, Factura: {grupo.Key.NoFacSae}");
                    }
        ...
        private List<ExcelCancelaProvisiones> ObtenerInformacionArchivo()
        {
            try
            {
                ...
                if (hoja.Cell("M1").Value.ToString() != "FormaCobro")
                {
                    validarNombresEncabezados = false;
                }
                if (hoja.Cell("N1").Value.ToString() != "TipoCambio")
                {
                    validarNombresEncabezados = false;
                }
                if (hoja.Cell("O1").Value.ToString() != "ImporteMXN")
                {
                    validarNombresEncabezados = false;
                }
                if (hoja.Cell("P1").Value.ToString() != "FechaInicio")
                {
                    validarNombresEncabezados = false;
                }
                if (hoja.Cell("Q1").Value.ToString() != "Mes")
                {
                    validarNombresEncabezados = false;
                }
                if (hoja.Cell("R1").Value.ToString() != "Anio")
                {
                    validarNombresEncabezados = false;
                }
                if (hoja.Cell("S1").Value.ToString() != "SectorCliente")
                {
                    validarNombresEncabezados = false;
                }
                if (hoja.Cell("T1").Value.ToString() != "TipoIngreso")
                {
                    validarNombresEncabezados = false;
                }
                if (hoja.Cell("U1").Value.ToString() != "KAM")
                {
                    validarNombresEncabezados = false;
                }
                if (hoja.Cell("V1").Value.ToString() != "NoFacSae")
                {
                    validarNombresEncabezados = false;
                }
                if (hoja.Cell("W1").Value.ToString() != "NombreProyecto")
                {
                    validarNombresEncabezados = false;
                }
                if (hoja.Cell("X1").Value.ToString() != "FechaDeCierre")
                {
                    validarNombresEncabezados = false;
                }
                ...
                List<ExcelCancelaProvisiones> lregistrosExcel = new List<ExcelCancelaProvisiones>();
                ExcelCancelaProvisiones registroExcel = new ExcelCancelaProvisiones();
                ...
                foreach (var fila in hoja.RowsUsed())
                {
                    ...
                    registroExcel.FormaCobro = hoja.Cell($"M{row}").Value.ToString();
                    registroExcel.TipoCambio = decimal.TryParse(hoja.Cell($"N{row}").Value.ToString(), out decimal tipoCambio) ? tipoCambio : throw new Exception($"El tipo de cambio en la fila {row} no es correcto");
                    registroExcel.ImporteMXN = decimal.TryParse(hoja.Cell($"O{row}").Value.ToString(), out decimal importeMXN) ? importeMXN : throw new Exception($"El importe MXN en la fila {row} no es correcto");
                    registroExcel.FechaInicio = DateTime.TryParse(hoja.Cell($"P{row}").Value.ToString(), out DateTime fechaInicio) ? fechaInicio : throw new Exception($"La fecha inicio {row} no es valida");
                    registroExcel.Mes = short.TryParse(hoja.Cell($"Q{row}").Value.ToString(), out short mes) ? mes : throw new Exception($"El mes {row} no es valido"); ;
                    registroExcel.Anio = short.TryParse(hoja.Cell($"R{row}").Value.ToString(), out short anio) ? anio : throw new Exception($"El año {row} no es valido"); ;
                    registroExcel.SectorCliente = hoja.Cell($"S{row}").Value.ToString();
                    registroExcel.TipoIngreso = hoja.Cell($"T{row}").Value.ToString();
                    registroExcel.KAM = hoja.Cell($"U{row}").Value.ToString();
                    registroExcel.NoFacSae = hoja.Cell($"V{row}").Value.ToString();
                    registroExcel.NombreProyecto = hoja.Cell($"W{row}").Value.ToString();
                    registroExcel.FechaDeCierre = DateTime.TryParse(hoja.Cell($"X{row}").Value.ToString(), out DateTime FechaDeCierre) ? FechaDeCierre : throw new Exception($"La fecha cierre en la fila {row} no es valida");
        ...
        private void guardarInformacionArchivoCierreDeMes(List<ExcelCancelaProvisiones> lregistrosExcel)
        {
            try
            {
                using (Elara_GestionFacturasEntities dbContext = new Elara_GestionFacturasEntities())
                {
                    using (DbContextTransaction transaccion = dbContext.Database.BeginTransaction())
                    {
                        try
                        {
                            CancelaProvision cierreDeMesRegistro = new CancelaProvision();
                            int contadorRegistros = 0;
                            foreach (ExcelCancelaProvisiones registroExcel in lregistrosExcel)
                            {

                                CancelaProvision registroActual = dbContext.CancelaProvision.Where(x => x.LineItemId == registroExcel.LineItemId &&
                                                                                                        x.Mes == registroExcel.Mes &&
                                                                                                        x.Anio == registroExcel.Anio &&
                                                                                                        x.NoFacSae == registroExcel.NoFacSae).FirstOrDefault();
                                ...
                                {
                                    cierreDeMesRegistro = new CancelaProvision();
                                    ...
                                    cierreDeMesRegistro.NoFacSae = registroExcel.NoFacSae;
                                    ...
                                    dbContext.CancelaProvision.Add(cierreDeMesRegistro);
                                }
                                else
                                {
